...
EXTRA = [
 G('Haynes 230','haynes-230','UNS N06230','Specialty','8.97','Ni ~57% · Cr ~22% · W ~14% · Mo ~2%','390 MPa','860 MPa','1150 °C',
   ['Outstanding high-temperature strength & oxidation resistance','Excellent thermal-fatigue resistance'],
   ['Gas turbine combustors','Heat-treat furnace hardware','Aerospace hot sections'],
   ['sheet','bar','wire'],
   'A tungsten-strengthened Ni-Cr-W alloy for the hottest oxidizing service.'),
 G('Haynes 188','haynes-188','UNS R30188','Specialty','9.13','Co bal. · Ni ~22% · Cr ~22% · W ~14%','445 MPa','960 MPa','1095 °C',
   ['Cobalt-base superalloy','Superior sulfidation resistance','Excellent fabricability'],
   ['Gas turbine transition ducts','Combustion liners','Afterburner parts'],
   ['sheet','bar','wire'],
   'The cobalt-base combustion alloy — strength and sulfidation resistance where nickel alloys soften.'),
 G('Haynes 25 (L605)','haynes-25','UNS R30605','Specialty','9.13','Co bal. · Cr ~20% · W ~15% · Ni ~10%','445 MPa','970 MPa','1095 °C',
   ['Cobalt-base wear & heat resistant alloy','Good galling resistance'],
   ['Turbine blades & vanes','Wear pads','Valve seats'],
   ['bar','sheet','wire'],
   'L605 — the classic cobalt alloy for hot wear and galling service.'),
 G('Waspaloy','waspaloy','UNS N07001','Specialty','8.19','Ni ~58% · Cr ~19% · Co ~13% · Mo ~4% · Ti ~3% · Al ~1.4%','725 MPa','1170 MPa (aged)','870 °C',
   ['Age-hardenable turbine superalloy','Higher temperature capability than 718'],
   ['Turbine discs & blades','Compressor hardware','Fasteners'],
   ['bar','forging','wire'],
   'The classic turbine-disc alloy — strength between 718 and the powder-metallurgy grades.'),
 G('Nimonic 80A','nimonic-80a','UNS N07080','Specialty','8.19','Ni ~76% · Cr ~20% · Ti ~2.3% · Al ~1.2%','620 MPa','1000 MPa (aged)','815 °C',
   ['Age-hardenable Ni-Cr-Ti alloy','Excellent creep & fatigue to 815 °C'],
   ['Turbine blades','Exhaust valves','Hot fasteners'],
   ['bar','wire'],
   'The exhaust-valve and turbine-blade classic from the Nimonic series.'),
 G('Nimonic 90','nimonic-90','UNS N07090','Specialty','8.18','Ni ~59% · Cr ~20% · Co ~17% · Ti ~2.5% · Al ~1.4%','690 MPa','1080 MPa (aged)','920 °C',
   ['Cobalt-bearing Nimonic for higher temperatures','Excellent stress-rupture life'],
   ['Turbine blades & discs','High-temperature springs'],
   ['bar','wire'],
   'A higher-temperature Nimonic for turbine hot sections.'),
 G('Nitronic 50','nitronic-50','UNS S20910','Specialty','7.89','Cr ~22% · Ni ~13% · Mn ~5% · Mo ~2.5% · N ~0.3%','450 MPa','825 MPa','—',
   ['Nitrogen-strengthened austenitic','Better strength + corrosion than 316L at stainless price','Non-magnetic'],
   ['Marine shafts & fasteners','Chemical equipment','High-strength corrosion hardware'],
   ['bar','wire'],
   'XM-19 — double the strength of 316L with better corrosion resistance.'),
 G('Nitronic 60','nitronic-60','UNS S21800','Specialty','7.62','Cr ~17% · Ni ~9% · Mn ~8% · Si ~4% · N ~0.1%','345 MPa','655 MPa','—',
   ['Outstanding galling & wear resistance','Austenitic stainless with silicon + nitrogen'],
   ['Valve stems & seats','Wear rings','Fasteners subject to galling'],
   ['bar','wire'],
   'The anti-galling stainless — where 304/316 seize, Nitronic 60 slides.'),
 G('Precision alloy 4J36 (Invar)','4j36-invar','UNS K93601','Specialty','8.1','Ni ~36% · Fe bal.','—','—','—',
   ['Near-zero thermal expansion below 200 °C','Dimensionally stable'],
   ['Precision instruments','Cryogenic structures','Bimetal strip components'],
   ['bar','sheet','wire','strip'],
   'Invar — the zero-expansion alloy for precision engineering.'),
 G('Precision alloy 4J42','4j42','UNS K94100','Specialty','8.12','Ni ~42% · Fe bal.','—','—','—',
   ['Expansion matched to soft glass & ceramics','Hermetic sealing alloy'],
   ['Glass-to-metal seals','Electronic packaging','Lead frames'],
   ['wire','strip','bar'],
   'The glass-sealing alloy for hermetic electronic packages.'),
 G('Precision alloy 1J50','1j50','—','Specialty','8.2','Ni ~50% · Fe bal.','—','—','—',
   ['Soft-magnetic with high permeability','Low coercive force'],
   ['Magnetic shielding','Transformer laminations','Relays'],
   ['strip','wire','bar'],
   'A 50% nickel soft-magnetic alloy for shielding and cores.'),
 G('Precision alloy 1J79','1j79','—','Specialty','8.6','Ni ~79% · Mo ~4% · Fe bal.','—','—','—',
   ['Very high initial permeability (permalloy class)','Exceptional magnetic shielding'],
   ['High-sensitivity magnetic shields','Current transformers','Magnetic heads'],
   ['strip','wire'],
   'A permalloy-class shielding alloy — the highest-permeability grade we supply.'),
 G('GH3030','gh3030','—','Specialty','8.4','Ni bal. · Cr ~20% · Ti ~0.3%','—','—','800 °C',
   ['Chinese-standard (GH) solid-solution Ni-Cr alloy','Oxidation resistant, good fabricability'],
   ['Combustion chambers','Heat-treat equipment'],
   ['bar','sheet','wire'],
   'A GH-series Ni-Cr alloy for 800 °C-class service.'),
 G('GH4169','gh4169','≈ UNS N07718','Specialty','8.24','Ni ~53% · Cr ~19% · Nb ~5% · Mo ~3% · Ti ~1%','1030 MPa','1240 MPa','700 °C',
   ['Chinese-standard equivalent of Inconel 718','Age-hardenable superalloy'],
   ['Aerospace turbine parts','High-strength fasteners'],
   ['bar','forging','wire'],
   'The GH-series counterpart to 718 — supplied to domestic and export aerospace programs.'),
 G('Nimonic 75','nimonic-75','UNS N06075','Specialty','8.37','Ni ~80% · Cr ~20% · Ti ~0.4%','—','—','—',
   ['Solid-solution Ni-Cr for high-temperature sheet work','Good oxidation resistance & fabricability'],
   ['Sheet fabrications for hot gas','Furnace parts'],
   ['sheet','bar','wire'],
   'A fabricable Nimonic for high-temperature sheet structures.'),
]
FAMILIES = [('Monel®', MONEL), ('Inconel®', INCONEL), ('Incoloy®', INCOLOY), ('Hastelloy®', HASTELLOY), ('Specialty & Pure Nickel', SPECIALTY + EXTRA)]
